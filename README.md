...
#    If sharing one Postgres DB with other sites, also set DB_SCHEMA=heritage_rch (or another unique schema name).
# 3) Add a PostgreSQL database in Render; set DATABASE_URL automatically via the dashboard or render.yaml.
#    The schema is auto-created by manage.py in production when DB_SCHEMA is set.
